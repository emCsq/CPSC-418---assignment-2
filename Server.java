...
public class Server
{
	...
	private static boolean debugFlagOn = false; //debug flag status
	
	//A set method that toggles the debug flag on should the method be called
	public static boolean turnDebugFlagOn(boolean flagOn) {
		debugFlagOn = flagOn;
		return debugFlagOn;
	}
	
	//A 'get' method to retrieve the status of the debug flag 
    ...
    {
	if (args.length != 1) {
		if (args[1].equalsIgnoreCase("debug")) {		//if debug is in the title, then call following method.
			turnDebugFlagOn(true);	//turn on the debug flag
		} else {
